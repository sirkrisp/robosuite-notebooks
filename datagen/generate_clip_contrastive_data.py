...
if __name__ == "__main__":
    ...
    output_folder = os.path.join(os.path.dirname(__file__), "clip_contrastive_01")
    ...
    images = np.memmap(
        ...
        dtype=np.uint8,
        mode="w+",
        shape=(num_envs, num_samples_per_env, *img_res, 3),
    )
    ...
    for i in tqdm.tqdm(range(num_envs)):
        ...
        for j in range(num_samples_per_env):
            images[i, j] = env.sim.render(*img_res, camera_name=camera_name)
